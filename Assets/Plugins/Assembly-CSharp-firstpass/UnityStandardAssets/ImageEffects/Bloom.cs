	...
	public class Bloom : PostEffectsBase
	{
		...
		public void OnRenderImage(RenderTexture source, RenderTexture destination)
		{
			...
			if (quality > BloomQuality.Cheap)
			{
				...
				if (screenBlend.passCount == 1)
					passes = 1;

				Graphics.Blit(source, temporary2, screenBlend, passes);
				RenderTexture temporary3 = RenderTexture.GetTemporary(width2, height2, 0, format);
				Graphics.Blit(temporary2, temporary3, screenBlend, passes);

				if (passes == 2)
					passes = 6;

				Graphics.Blit(temporary3, temporary, screenBlend, passes);
			...
			else
			{
				...
				Graphics.Blit(temporary2, temporary, screenBlend, passes);
			}
			...
			for (int i = 0; i < bloomBlurIterations; i++)
			{
				...
				if (blurAndFlaresMaterial.passCount == 1)
					passes = 1;
				
				Graphics.Blit(renderTexture, temporary4, blurAndFlaresMaterial, passes);
			...
			if (quality > BloomQuality.Cheap)
			{
				...
				Graphics.Blit(temporary, renderTexture, screenBlend, passes);
			}
			if (lensflareIntensity > Mathf.Epsilon)
			{
				...
				if (lensflareMode == LensFlareStyle.Ghosting)
				{
					...
					if (quality > BloomQuality.Cheap)
					{
						...
						Graphics.Blit(temporary5, temporary, blurAndFlaresMaterial, passes);
						...
						GL.Clear(false, true, Color.black);
						Graphics.Blit(temporary, temporary5, blurAndFlaresMaterial, passes);
					}
		...
		private void AddTo(float intensity_, RenderTexture from, RenderTexture to)
		{
			...
			Graphics.Blit(from, to, screenBlend, passes);
		}
		...
		private void Vignette(float amount, RenderTexture from, RenderTexture to)
		{
			if ((bool)lensFlareVignetteMask)
			{
				...
				int passes = (!(from == to)) ? 3 : 7;

				if (screenBlend.passCount == 1)
					passes = 1;

                Graphics.Blit((!(from == to)) ? from : null, to, screenBlend, passes);
			}
			else if (from != to)
			{
