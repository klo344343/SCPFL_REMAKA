	...
	public class DepthOfField : PostEffectsBase
	{
		...
		private void WriteCoc(RenderTexture fromTo, bool fgDilate)
		{
			...
			if (nearBlur && fgDilate)
			{
				...
				if (dofHdrMaterial.passCount == 1)
					passes = 1;
				
				Graphics.Blit(fromTo, temporary, dofHdrMaterial, passes);
				...
				Graphics.Blit(temporary, temporary2, dofHdrMaterial, passes);
				...
				Graphics.Blit(temporary2, temporary, dofHdrMaterial, passes);
				...
				Graphics.Blit(fromTo, fromTo, dofHdrMaterial, passes);
				RenderTexture.ReleaseTemporary(temporary);
			}
			else
			{
		...
		private void OnRenderImage(RenderTexture source, RenderTexture destination)
		{
            ...
            if (visualizeFocus)
			{
				...
				Graphics.Blit(source, destination, dofHdrMaterial, passes);
			}
			else if (blurType == BlurType.DX11 && (bool)dx11bokehMaterial)
			{
				if (highResolution)
				{
					...
					Graphics.Blit(source, renderTexture3, dofHdrMaterial, passes);
					dofHdrMaterial.SetVector("_Offsets", new Vector4(0f, 1.5f, 0f, 1.5f));

					if (passes != 1)
						passes = 19;

					Graphics.Blit(renderTexture3, renderTexture4, dofHdrMaterial, passes);
					dofHdrMaterial.SetVector("_Offsets", new Vector4(1.5f, 0f, 0f, 1.5f));
					Graphics.Blit(renderTexture4, renderTexture3, dofHdrMaterial, passes);
					if (nearBlur)
					{
						if (passes != 1)
							passes = 4;
					...
					if (nearBlur)
					{
						...
						Graphics.Blit(renderTexture4, renderTexture, dofHdrMaterial, passes);
					}

					if (passes != 1)
						passes = 20;

					Graphics.Blit(renderTexture, temporary, dofHdrMaterial, passes);
					dofHdrMaterial.SetVector("_Offsets", new Vector4(internalBlurWidth, 0f, 0f, internalBlurWidth));

					if (passes != 1)
						passes = 5;
					
					Graphics.Blit(renderTexture, source, dofHdrMaterial, passes);
					dofHdrMaterial.SetVector("_Offsets", new Vector4(0f, internalBlurWidth, 0f, internalBlurWidth));

					if (passes != 1)
						passes = 21;
					
					Graphics.Blit(source, temporary, dofHdrMaterial, passes);
				...
				else
				{
					...
					Graphics.Blit(source, renderTexture, dofHdrMaterial, passes);
					...
					Graphics.Blit(renderTexture, renderTexture3, dofHdrMaterial, passes);
					dofHdrMaterial.SetVector("_Offsets", new Vector4(0f, 1.5f, 0f, 1.5f));
					...
					if (nearBlur)
					{
						...
						Graphics.Blit(source, renderTexture5, dofHdrMaterial, passes);
					}
					dx11bokehMaterial.SetTexture("_BlurredColor", renderTexture3);
					dx11bokehMaterial.SetFloat("_SpawnHeuristic", dx11SpawnHeuristic);
					dx11bokehMaterial.SetVector("_BokehParams", new Vector4(dx11BokehScale, dx11BokehIntensity, Mathf.Clamp(dx11BokehThreshold, 0.005f, 4f), internalBlurWidth));
					...
					if (nearBlur)
					{
						...
						Graphics.Blit(renderTexture5, renderTexture, dofHdrMaterial, passes);
						dofHdrMaterial.SetVector("_Offsets", new Vector4(num, 0f, 0f, num));
						Graphics.Blit(renderTexture, renderTexture5, dofHdrMaterial, passes);

						if (passes != 1)
							passes = 3;

						Graphics.Blit(renderTexture5, renderTexture2, dofHdrMaterial, passes);
					}
					dofHdrMaterial.SetVector("_Offsets", new Vector4(internalBlurWidth, 0f, 0f, internalBlurWidth));
					...
					Graphics.Blit(renderTexture2, renderTexture, dofHdrMaterial, passes);
					dofHdrMaterial.SetVector("_Offsets", new Vector4(0f, internalBlurWidth, 0f, internalBlurWidth));
					Graphics.Blit(renderTexture, renderTexture2, dofHdrMaterial, passes);
			...
			else
			{
				...
				else
				{
					...
					if (passes != 1)
						passes = (blurSampleCount != BlurSampleCount.High) ? 12 : 18;
