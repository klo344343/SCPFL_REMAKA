...
namespace Dissonance
{
	public class Log
	{
        private static readonly LogLevel[] _categoryLogLevels;
        ...
        public static Log GetLog(LogCategory category, string name)
        {
            return new Log((int)category, name);
        }

        public static Log GetLog(string name)
        {
            return new Log((int)LogCategory.Core, name);
        }

        internal static LogLevel GetLogLevel(int category)
        {
            if (category >= 0 && category < _categoryLogLevels.Length)
            {
                return _categoryLogLevels[category];
            }
        ...
        public static void SetLogLevel(LogCategory category, LogLevel level)
        {
            ...
            if (catIdx >= 0 && catIdx < _categoryLogLevels.Length)
            {
                _categoryLogLevels[catIdx] = level;
            }
        }
